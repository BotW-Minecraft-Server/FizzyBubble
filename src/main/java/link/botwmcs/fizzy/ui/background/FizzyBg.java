...
public class FizzyBg implements BgPainter {
    ...
    public void paint(GuiGraphics g, FramePainter painter) {
        ...
        switch (type) {
            case PURE_GRAY -> {
                tile(g, TEX_PURE_GRAY, x, y, w, h, 16, 16);
            }
            case STONE -> {
                tile(g, TEX_STONE, x, y, w, h, 16, 16);
            }
            case BARRIER -> {
                tile(g, TEX_BARRIER, x, y, w, h, 16, 16);
            }
            case BARRIER_BLUE -> {
                tile(g, TEX_BARRIER_BLUE, x, y, w, h, 16, 16);
            }
            case BOTW -> {
                tile(g, TEX_BOTW, x, y, w, h, 16, 16);
            }
        }
    }
    ...
    private static void tile(GuiGraphics g, ResourceLocation tex,
                             int x, int y, int w, int h, int tileW, int tileH) {
        int yy = y, remH = h;
        while (remH > 0) {
